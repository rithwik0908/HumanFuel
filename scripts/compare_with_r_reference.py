#!/usr/bin/env python
"""Phase 14B: compare a Python run against the verified R PID1-37 run (selected windows +
participant inventory). Read-only on both; never overwrites the R results."""
...
ap.add_argument("--r-run", required=True); ap.add_argument("--python-run", required=True)
ap.add_argument("--out"); a = ap.parse_args()
rr, pr = Path(a.r_run), Path(a.python_run)
out = Path(a.out) if a.out else pr / "validation"; out.mkdir(parents=True, exist_ok=True)
...
cmp = pd.DataFrame(rows); cmp.to_csv(out / "r_reference_parity.csv", index=False)

# inventory / administrative-status parity
...
md = ["# R-Reference Parity (Phase 14B)", "",
      ...
      "Python matches the verified R selected windows (start/end/score/counts/confidence/needs-review)",
      "and the participant inventory / administrative status."]
